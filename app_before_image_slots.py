...
def edit_house(house_id):
    ...
    if request.method == "POST":
        ...
        # استبدال الصور فقط إذا اختار المستخدم صورًا جديدة
        images = request.files.getlist("images")

        new_images = [
            image for image in images[:3]
        ...
        if new_images:
            ...
            # إذا اختار صورًا أقل من 3، نمسح الخانات الزائدة
            for i in range(len(new_images) + 1, 4):
                setattr(house, f"image{i}", None)
